...
void kernel convolve_imagecubes_float2(
        const int numExamples, global const float *inputs,
        global const float *filters, global float *output,
        const int gOutputSizeSquared, const int gNumFilters,
    ...
    if (exampleId < numExamples) {
        for (int inputPlaneIdx = 0; inputPlaneIdx < gNumInputPlanes; inputPlaneIdx++) {
            ...
            for (int u = -gHalfFilterSize; u <= gHalfFilterSize - gEven; u++) {
                ...
                #endif
                global float const *inputRow = inputPlane + (outputRow + u) * gInputSize;
                global float const *filterRow = filterPlane + (u+gHalfFilterSize) * gFilterSize + gHalfFilterSize;
                bool rowOk = (outputRow + u) >= 0 && (outputRow + u) < gInputSize;
                #pragma unroll
